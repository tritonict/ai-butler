...
export default function Header({ toggleSidebar }: { toggleSidebar?: () => void }) {
  ...
  return (
  <>
    <header className="overflow-x-hidden flex items-center justify-between bg-gray-100 dark:bg-neutral-900 text-zinc-900 dark:text-white px-3 sm:px-2 lg:px-4 py-2 text-sm relative">
        
          <button onClick={toggleSidebar} className="block">
            <Menu className="w-6 h-6 shrink-0 text-neutral-800 dark:text-white" />
          </button>
          <img src="/logo_black.png" alt="Logo" className="h-8 w-8 shrink 0" />
        
        <h1 className="hidden md:block text-xl font-bold text-neutral-800 dark:text-white">AI Butler</h1>
      
        <span onClick={toggleView} className="hidden md:block cursor-pointer hover:underline">
          {tokenText} tokens
        </span>
        <span className="hidden md:block ">€ {formattedSaldo}</span>
